...
/**
 * Ripped from Scalatra on 5/9/14.
 */
...
object SwaggerSerializers {
  ...
  private implicit class RichString(orig: String) extends AnyRef {
  ...
  class HttpMethodSerializer extends CustomSerializer[Method](implicit formats => ({
    case JString(method) => Method.fromKey(method)
  },{
    case method: Method => JString(method.name)
  }))
  ...
  class ModelSerializer extends CustomSerializer[Model](implicit formats => ({
    ...
    case x: Model =>
      ...
      ("id" -> x.id) ~
        ...
        ("required" -> required) ~
        ("extends" -> x.baseModel.filter(s => !s.isBlank && !s.trim.equalsIgnoreCase("VOID"))) ~
  ...
  class OperationSerializer extends CustomSerializer[Operation](implicit formats => ({
    case value =>
      Operation(
        (value \ "method").extract[Method],
        readDataType(value),
        (value \ "summary").extract[String],
        (value \ "nickname").extract[String],
