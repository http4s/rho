...
class TypeBuilderSpec extends Specification {
  ...
  "TypeBuilder" should {
    ...
    "Build a composite model" in {
      ...
      m1.properties.get("many") must beLike { case Some(array: ArrayProperty) =>
        array.items must beLike { case ref: RefProperty =>
          ref.ref must_== "Foo"
        }
      }
    ...
    "Build model that contains a List" in {
      ...
      m.properties.head._2 must beLike { case items: ArrayProperty =>
    ...
    "Build model that contains a List of Lists" in {
      ...
      m.properties.head._2 must beLike { case items: ArrayProperty =>
        ...
        items.items must beLike { case items: ArrayProperty =>
          items.`type` must_== "array"
    ...
    "Build a model with the underlying type for AnyVals" in {
      ...
      m.properties.size must_== 1
      m.properties.head must beLike { case (name, prop: RefProperty) =>
        name must_== "fooVal"
        prop.ref must_== "Foo"
      }
    }
