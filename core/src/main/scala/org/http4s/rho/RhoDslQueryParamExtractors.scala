...
trait RhoDslQueryParamExtractors[F[_]] extends FailureResponseOps[F] {
  ...
  private def _paramR[T](
    ...
    }.withMetadata(QueryMetaData(name, description, parser, default = default, m, enum))
  ...
  /** Defines an enum parameter in query string that should be bound to a route definition.
    ...
    */
  def paramE[T](name: String, default: T, enum: Seq[String])(implicit
    F: FlatMap[F],
    parser: QueryParser[F, T],
    m: TypeTag[T]): TypedQuery[F, T :: HNil] =
    _paramR(name, None, Some(default), _ => None, enum)
  ...
  /** Defines an enum parameter in query string that should be bound to a route definition.
    ...
    * @param enum list of allowed values
    */
  def paramE[T](name: String, enum: Seq[String])(implicit
    F: FlatMap[F],
    parser: QueryParser[F, T],
    m: TypeTag[T]): TypedQuery[F, T :: HNil] =
    _paramR(name, None, None, _ => None, enum)
