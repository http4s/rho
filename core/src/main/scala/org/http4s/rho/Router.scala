...
case class CodecRouter[T <: HList, R](router: Router[T], decoder: EntityDecoder[IO, R])(implicit t: TypeTag[R])
    ...
    with Decodable[T, R]
{
  ...
  override def decoding[R2 >: R](decoder2: EntityDecoder[IO, R2])(implicit t: TypeTag[R2]): CodecRouter[T, R2] =
