...
trait RouteExecutable[T <: HList] extends TypedBuilder[T] {
  ...
  /** Provide an action from which to generate a complete route
    ...
    */
  final def runWith[F, O](f: F)(implicit hf: HListToFunc[T, F]): Request => Task[Option[Response]] = {
    val srvc = new RhoService { compilerSrvc.compile(makeAction(f, hf)) }.toService
