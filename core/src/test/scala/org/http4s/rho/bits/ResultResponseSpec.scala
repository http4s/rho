...
class ResultResponseSpec extends Specification {
  ...
  "ResultResponse" should {
    ...
    "map a FailureResponse" in {
      val result: ResultResponse[Int] = FailureResponse.badRequest("foo")
      result.map(_.toString) should_== result
    }

    "flatMap a SuccessResponse" in {
      SuccessResponse(3).flatMap(i => SuccessResponse(i.toString)) should_== SuccessResponse("3")
    }
    ...
    "work in a failing for comprehensions" in {
      val a: ResultResponse[String] = FailureResponse.badRequest("boo")
      val b: ResultResponse[String] = SuccessResponse("bar")
      val result1 = for {
        foo <- a
        bar <- b
      } yield (foo + bar)
