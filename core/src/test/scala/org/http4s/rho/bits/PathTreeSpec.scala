  ...
  "PathTree mergers" >> {
    ...
    "MatchNodes" should {
      ...
      "Merge non-empty intermediate nodes with matching paths" in {
        val n1 =
          MatchNode("foo", matches = Map("bar" -> MatchNode("bar", end = Map(Method.GET -> l))))
        val n2 =
          MatchNode("foo", matches = Map("bar" -> MatchNode("bar", end = Map(Method.POST -> l))))
        val r = MatchNode(
          "foo",
          matches = Map("bar" -> MatchNode("bar", end = Map(Method.POST -> l, Method.GET -> l)))
        )
    ...
    "CapturesNodes" should {
      ...
      "Merge non-empty intermediate nodes with matching paths" in {
        val n1 =
          CaptureNode(p, matches = Map("bar" -> MatchNode("bar", end = Map(Method.GET -> l))))
        val n2 =
          CaptureNode(p, matches = Map("bar" -> MatchNode("bar", end = Map(Method.POST -> l))))
        val r = CaptureNode(
          p,
          matches = Map("bar" -> MatchNode("bar", end = Map(Method.POST -> l, Method.GET -> l)))
        )
