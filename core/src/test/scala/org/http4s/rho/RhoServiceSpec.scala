...
class RhoServiceSpec extends Specification with RequestRunner {
  ...
  val service = new RhoService {
    GET +? param("foo", "bar") |>> AAction{ foo: String => Ok(s"just root with parameter 'foo=$foo'") }

    GET / "" +? param("foo", "bar") |>> AAction{ foo: String => Ok("this definition should be hidden by the previous definition") }

    GET / "hello" |>> AAction(Ok("route1"))

    GET / 'hello |>> AAction{ hello: String => Ok("route2") }

    GET / "hello" / "world" |>> AAction(Ok("route3"))

    GET / "hello" / "headers" +? param[Int]("foo") |>> AAction { foo: Int => Ok("route" + foo) }

    GET / "hello" / "default" / "parameter" +? param[Int]("some", 23) |>> AAction { s: Int => Ok("some:" + s) }

    // Routes that will have different headers/query string requirements should work together
    GET / "hello" / "compete" +? param[Int]("foo") |>> AAction { foo: Int => Ok("route" + foo) }

    GET / "hello" / "compete" +? param[String]("foo") |>> AAction { foo: String => Ok("route6_" + foo) }

    GET / "hello" / "compete" |>> AAction(Ok("route7"))

    // Testing query params
    GET / "query" / "twoparams" +? param[Int]("foo") & param[String]("bar") |>> AAction { (foo: Int, bar: String) =>
      Ok("twoparams" + foo + bar)
    }

    GET / "query" / "twoparams2" +? param[Int]("foo") & param[Option[String]]("bar") |>> AAction { (foo: Int, bar: Option[String]) =>
      Ok("twoparams2_" + foo + bar.getOrElse("cat"))
    }

    GET / "variadic" / * |>> AAction { tail: Seq[String] => Ok("route8_" + tail.mkString("/")) }

    val or = "or1" || "or2"
    GET / or |>> AAction { () => Ok("route9") }

    GET / "orders" / pathVar[Int]("id") |>> AAction { id: Int => Ok(id.toString) }

    GET / "options" +? param[Option[String]]("foo") |>> AAction { os: Option[String] => Ok(os.getOrElse("None")) }

    GET / "seq" +? param[Seq[String]]("foo") |>> AAction { os: Seq[String] => Ok(os.mkString(" ")) }

    GET / "seq" +? param[Seq[Int]]("foo") |>> AAction { os: Seq[Int] => Ok(os.mkString(" ")) }

    GET / "withreq" +? param[String]("foo") |>> AAction { (req: Request, foo: String) => Ok(s"req $foo") }

    val rootSome = root / "some"
    GET / rootSome |>> AAction (Ok("root to some"))

    GET / "directTask" |>> AAction {
      val i = new AtomicInteger(0)
      Task(s"${i.getAndIncrement}")
    }

    GET / "terminal" / "" |>> AAction("terminal/")

    GET / "terminal" |>> AAction("terminal")

    GET / "one" / "two" / "three" |>> AAction ("one/two")
  }
  ...
  "RhoService" should {
    ...
    "Attempt to evaluate params in order" in {
      val service = new RhoService {
        GET / "foo" +? param[Int]("bar") |>> AAction { i: Int => Ok(s"Int: $i") }
        GET / "foo" +? param[String]("bar") |>> AAction { i: String => Ok(s"String: $i") }
        GET / "foo" |>> AAction (Ok("none"))
      }.toService
    ...
    "Match an empty Option over a bare route" in {
      val service = new RhoService {
        GET / "foo" +? param[Option[String]]("bar") |>> AAction { o: Option[String] =>
          o.map(s => s"String: $s").getOrElse("none")
        }

        GET / "foo" |>> AAction(Ok(s"failure"))
      }.toService
    ...
    "work with all syntax elements" in {
      ...
      val srvc = new RhoService {
        POST / "foo" / pathVar[Int] +? param[String]("param") >>> reqHeader ^ EntityDecoder.text |>> AAction{
  ...
  ////////////////////////////////////////////////////
    "Handle errors in the route actions" in {
      val service = new RhoService {
        GET / "error" |>> AAction{ () => throw new Error("an error"); Ok("Wont get here...") }
      }.toService
