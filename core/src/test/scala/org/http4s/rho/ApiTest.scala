...
class ApiTest extends Specification {
  ...
  "RhoDsl bits" should {
    ...
    "Append headers to a Route" in {
      ...
      val route = (path >>> validations >>> capture(headers.ETag)).decoding(EntityDecoder.text) runWith AAction {
    ...
    "Run || routes" in {
      ...
      val f = GET / (p1 || p2) runWith AAction { (s: String) => Ok("").withHeaders(headers.ETag(s)) }
    ...
    "Execute a complicated route" in {
      ...
      val route =
        (path >>> validations).decoding(EntityDecoder.text) runWith AAction {(world: String, fav: Int, tag: headers.ETag, body: String) =>
    ...
    "Deal with 'no entity' responses" in {
      val route = GET / "foo" runWith AAction { () => SwitchingProtocols() }
  ...
  "PathValidator" should {
    ...
    "traverse a captureless path" in {
      ...
      val f = stuff runWith AAction { () => Ok("Cool.").withHeaders(headers.ETag("foo")) }
      check(f(req), "foo")
    }
    ...
    "capture a variable" in {
      ...
      val f = stuff runWith AAction { str: String => Ok("Cool.").withHeaders(headers.ETag(str)) }
      check(f(req), "hello")
    }
    ...
    "capture end with nothing" in {
      val stuff = GET / "hello" / *
      val req = Request(uri = Uri.fromString("/hello").getOrElse(sys.error("Failed.")))
      val f = stuff runWith AAction { path: List[String] => Ok("Cool.").withHeaders(headers.ETag(if (path.isEmpty) "go" else "nogo")) }
    ...
    "capture remaining" in {
      ...
      val f = stuff runWith AAction { path: List[String] => Ok("Cool.").withHeaders(headers.ETag(path.mkString)) }
  ...
  "Query validators" should {
    "get a query string" in {
      ...
      val route = path runWith AAction { i: Int => Ok("stuff").withHeaders(headers.ETag((i + 1).toString)) }
  ...
  "Decoders" should {
    "Decode a body" in {
      ...
      val route = path.decoding(EntityDecoder.text) runWith AAction { str: String =>
    ...
    "Allow the infix operator syntax" in {
      ...
      val route = path ^ EntityDecoder.text runWith AAction { str: String =>
        Ok("stuff").withHeaders(headers.ETag(str))
      }
    ...
    "Fail on a header" in {
      ...
      val route1 = path.validate(reqHeader) runWith AAction { () =>
      ...
      val route2 = path.validate(reqHeaderR) runWith AAction { () =>
        Ok("shouldn't get here.")
      }
    ...
    "Fail on a query" in {
      ...
      val route1 = (path +? param[Int]("foo")) runWith AAction{ i: Int =>
        Ok("shouldn't get here.")
      }

      route1(req).run.get.status should_== Status.BadRequest

      val route2 = (path +? paramR[String]("foo", (_: String) => Some(Unauthorized("foo")))) runWith AAction{ str: String =>
        Ok("shouldn't get here.")
      }
