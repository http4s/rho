  ...
  "mock api" should {
    "Make it easy to compose routes" in {
      ...
      new RhoService {
        ...
        path |>> AAction { () => Ok("Empty") } // use the |>> operator to turn a Router into an Action
        path2 |>> AAction { (world: String, fav: Int) => Ok(s"Received $fav, $world") }
        path2 |>> AAction(foo(_, _))
        ...
        GET / (path6 || path7) +? param[String]("foo") >>> (v6 || v7) |>> AAction {
          (i: Int, foo: String, v: Int) =>
            Ok(s"Received $i, $foo, $v")
        }

        // If you want to access the the Request, just add it as the first param
        GET / "getrequest" |>> AAction { req: Request => Ok("Dont need a request") }
        GET / "getrequest" / 'foo |>> AAction { (req: Request, foo: String) => Ok("I wanted a request") }

        // It is not manditory to give a status type
        GET / "nostatus" |>> AAction { () => "No status!"}
        GET / "taskNoStatus" |>> AAction { () => Task("A Future status!") }

        // You can also return a constant result for routes that are invariante of their inputs
        GET / "nostatus2" |>> AAction("This is a constant result!")
        GET / "taskNoStatus2" |>> AAction(Task("This task will be evaluated each time!"))

        // TODO: this is just a basic example!
        GET /"twoResults" |>> AAction { () =>
          if (true) Ok("bytes result".getBytes())
          else NotFound("Boo... Not found...")
        }

        // Using decoders you can parse the body as well
        POST / "postSomething" ^ UrlForm.entityDecoder |>> AAction { m: UrlForm =>
